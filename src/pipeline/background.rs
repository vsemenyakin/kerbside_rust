...
impl BackgroundModel {
    pub fn new(settings: &Settings, shape: (i32, i32)) -> Result<Self, String> {
        ...
        .map_err(|e| format!("{}{e}", crate::obfstr_err!("cannot create the background subtractor: ")))?;
        ...
        .map_err(|e| format!("{}{e}", crate::obfstr_err!("cannot build the opening kernel: ")))?;
        let close_kernel = opencv::imgproc::get_structuring_element(
            opencv::imgproc::MORPH_ELLIPSE,
            Size::new(bg.CLOSE_KERNEL, bg.CLOSE_KERNEL),
            CvPoint::new(-1, -1),
        )
        .map_err(|e| format!("{}{e}", crate::obfstr_err!("cannot build the closing kernel: ")))?;
    ...
    pub fn apply(&mut self, working: &Mat, pf: &mut perf::Frame) -> Result<(Mat, f64), String> {
        ...
        if self.shadow_value != 0 {
            ...
            .map_err(|e| format!("{}{e}", crate::obfstr_err!("cannot threshold the shadow value: ")))?;
            mask = thresholded;
        }
        ...
        .map_err(|e| format!("{}{e}", crate::obfstr_err!("cannot open the foreground mask: ")))?;
        ...
        .map_err(|e| format!("{}{e}", crate::obfstr_err!("cannot close the foreground mask: ")))?;
        mask = closed;
        pf.end(crate::perf::stage::MORPH);

        let set = opencv::core::count_non_zero(&mask)
            .map_err(|e| format!("{}{e}", crate::obfstr_err!("cannot measure the foreground: ")))?;
...
pub fn to_working(full: &Mat, width: i32, height: i32) -> Result<Mat, String> {
    ...
    .map_err(|e| format!("{}{e}", crate::obfstr_err!("cannot build the working frame: ")))?;
    Ok(working)
}
...
pub fn empty_mask(height: i32, width: i32) -> Result<Mat, String> {
    Mat::new_rows_cols_with_default(height, width, opencv::core::CV_8UC1, Scalar::all(0.0))
        .map_err(|e| format!("{}{e}", crate::obfstr_err!("cannot allocate an empty mask: ")))
}
