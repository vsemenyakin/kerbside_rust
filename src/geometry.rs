...
impl Homography {
    ...
    pub fn from_survey(
        ...
        if image_points.len() != 4 || world_points.len() != 4 {
            return Err(crate::obfstr_err!("a homography needs exactly four survey marks").into());
        }
        ...
        let mat = opencv::imgproc::get_perspective_transform_def(&source, &target)
            .map_err(|e| format!("{}{e}", crate::obfstr_err!("getPerspectiveTransform failed: ")))?;
        ...
        for (r, row) in to_world.iter_mut().enumerate() {
            for (c, cell) in row.iter_mut().enumerate() {
                *cell = *mat
                    ...
                    .map_err(|e| format!("{}({r},{c}): {e}", crate::obfstr_err!("homography element ")))?;
            }
        }
...
fn invert_3x3(m: &[[f64; 3]; 3]) -> Result<[[f64; 3]; 3], String> {
    ...
    for k in 0..N {
        ...
        if lu[p][k] == 0.0 {
            return Err(crate::obfstr_err!("the survey marks are collinear: the homography is singular").into());
        }
        if p != k {
            lu.swap(k, p);
            piv.swap(k, p);
...
mod tests {
    ...
    fn survey() -> Homography {
        ...
        .expect(crate::obfstr_err!("the declared survey must fit"))
    }
